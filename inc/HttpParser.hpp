...
/*   Updated: 2025/09/16 18:01:07 by fdi-cecc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
class HttpParser {
	...
	public:
		...
		static ServerData const &checkIfServerExist(
			std::vector<ServerData> const &servers,
			std::pair<int, std::string> incoming);
		static void checkIfPathExist(
			std::pair<std::string, std::string> const &path,
			LocationConf &blockLoc, std::string const &method);
		static void notAllowedMethod(
			std::map<std::string, LocationConf>::iterator loc,
			std::vector<std::string> const &serv_meth, std::string const &meth);

		static std::pair<std::string, std::string> parseHost(
			std::string const &str);
		static bool checkIfHostNameExistInServer(
			std::string &host, std::vector<std::string> const &serv);
		static std::pair<std::string, std::string> parseHeaderSyntaxis(
			std::string h);
		...
		static int parseContentLengthHeader(std::string const &v,
											std::size_t body_max);
		static std::string parseContentTypeBoundary(
			std::vector<std::string> const &v);
		static LocationConf const *findLocation(
