...
/*   Updated: 2025/09/14 13:27:05 by fdi-cecc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Response::setContent(std::pair<std::string, std::string> fullPath,
		...
		_statusCode = _request->getStatusCode();
		_method = method;
		if (_statusCode >= 400) return;
		std::cout << PINK << "FullPath.first(set content): " << fullPath.first
				  << "\n"
				  << "FullPath.second: " << fullPath.second << RESET
				  << std::endl;	 // TO BORROW
				...
				default:
						...
						if (!errorPages.empty()) {
								std::map<int, std::string>::const_iterator
									errorPageIt = errorPages.find(_statusCode);
								if (errorPageIt != errorPages.end())
										_location =
											_request->getFullPath().first +
											errorPageIt->second;
						} else {
								ServerData serv =
									HttpParser::checkIfServerExist(
										this->_request->getServersList(),
										incoming);
								std::map<int, std::string> const errorPages2 =
									serv.getErrorPage();

								std::map<int, std::string>::const_iterator
									errorPageIt = errorPages2.find(_statusCode);
						...
						std::cout << "LOCATIONNNNNNN: " << _location
								  << std::endl;
						_contentType = "text/html";
						content = prepFile();
						...
						replaceContent(content, "{{REASON_PHRASE}}",
									   reasonPhrase);
						break;
		}
}
...
void Response::sendResponse() {
		...
		while (totalSent < totalSize && attempts < maxAttempts) {
				...
				if (sent > 0)
						...
						std::cerr << "Connection closed by client during send"
								  << std::endl;
						break;
				} else {
						usleep(1000);
		...
		if (totalSent != totalSize) printBoxError("Warning, send incomplete");
}
