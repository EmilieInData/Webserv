...
void ServerManager::servSetup() {
		...
		graEmptyLine();
		for (std::set<std::pair<int, std::string> >::iterator it =
				 _uniqueListens.begin();
...
void ServerManager::servListen(std::pair<int, std::string> _listens) {
		...
		if (newsocket < 0) return graError("Socket creation error");
		...
		if (bind(newsocket, (struct sockaddr *)&newaddr, sizeof(newaddr)) < 0) {
				close(newsocket);
				return graError("Binding error for " + _listens.second + ":" +
								intToString(_listens.first));
		}

		if (listen(newsocket, 128) < 0) {
				close(newsocket);
				return graError("Listen error for " + _listens.second + ":" +
								intToString(_listens.first));
		}
...
void ServerManager::servRun() {
				...
				for (size_t i = 0; i < _polls.size(); ++i) {
						...
						if (_polls[i].revents &
							(POLLHUP | POLLERR | POLLNVAL)) {
								if (is_listener)
										printBoxError(
											"Error on listener socket " +
											intToString(_polls[i].fd));
								else
		...
		_clients.clear();
		for (size_t i = 0; i < _socketFd.size(); ++i) close(_socketFd[i]);
}
...
void ServerManager::handleNewConnection(int listener_fd) {
		...
		int client_fd =
			accept(listener_fd, (struct sockaddr *)&client_addr, &client_len);

		if (client_fd < 0) {
				if (errno != EAGAIN && errno != EWOULDBLOCK)
						printBoxError("Accept error: " +
									  std::string(strerror(errno)));
				return;
		}

		if (fcntl(client_fd, F_SETFL, O_NONBLOCK) < 0) {
				printBoxError("Fcntl error on new client fd " +
		...
		_clients[client_fd] =
			new Client(client_fd, client_addr, *this, incoming);
...
void ServerManager::handleClientRead(int client_fd) {
		...
		if (bytes <= 0) {
				if (bytes == 0)
						printBoxMsg("Client " + intToString(client_fd) +
									" disconnected gracefully.");
				else
						printBoxError("Recv error on client " +
									  intToString(client_fd) + ": " +
									  std::string(strerror(errno)));
		...
		if (it->second->request.getParsingState() <= 0) {
				...
				if (it->second->request.getStatusCode() < 400)
						resp.setContent(it->second->request.getFullPath(),
										it->second->request.getHttpMethod());
...
void ServerManager::handleClientWrite(int client_fd) {
		...
		ssize_t sent =
			send(client_fd, resp.c_str() + bytes_sent, len_to_send, 0);

		if (sent < 0) {
				printBoxError("Send error on client " + intToString(client_fd) +
							  ": " + std::string(strerror(errno)));
				removeClient(client_fd);
				return;
		}
		...
		if (bytes_sent == resp.length()) {
				_rspCount++;
				printResponse(*this, getSocketData(client_fd), resp,
							  it->second->request.getFullPath().first);
				removeClient(client_fd);
		}
}
...
std::pair<int, std::string> ServerManager::getSocketData(int socketFd) {
		...
		if (getsockname(socketFd, (struct sockaddr *)&socketIn, &socketInLen) <
			0)
				std::cerr << timeStamp() << "getsockname failed for socket num "
						  << socketFd << std::endl;
