...
/*   Updated: 2025/09/10 16:01:16 by fdi-cecc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								  ...
								  LocationConf &blockLoc, std::string const &method)
{
	...
	if (method == "DELETE") //FABIO added throw for failure
	{
		...
		{
			throw std::invalid_argument(E_405); // Method not allowed
		}

		if (std::remove(full.c_str()) != 0)
		{
			throw std::invalid_argument(E_403); // Forbidden - couldn't delete
		}

		throw std::invalid_argument(E_204); // No Content - successful deletion
	}
}
...
void HttpParser::notAllowedMethod(LocationConf const			 &location,
								  std::vector<std::string> const &allowed_serv,
								  std::string const				 &method)
{

	std::vector<std::string> const &allowed_loc = location.getAllowedMethods();

	// 1. Check location-specific methods first.
	// If this list exists, it is the definitive rule for this path.
	if (!allowed_loc.empty())
	{
		for (size_t i = 0; i < allowed_loc.size(); i++)
		{
			if (allowed_loc[i] == method)
				return; // Method is explicitly allowed here.
		}
		// If we finished the loop and didn't find the method, it's forbidden.
		throw std::invalid_argument(E_405);
	}
	// 2. If the location has no specific methods, fall back to server-wide methods.
	else if (!allowed_serv.empty())
	{
		for (size_t i = 0; i < allowed_serv.size(); i++)
		{
			if (allowed_serv[i] == method)
				return; // Method is allowed by the server default.
		}
		// If the server has a list and the method isn't in it, it's forbidden.
		throw std::invalid_argument(E_405);
	}
	// 3. (Optional but Recommended) If no methods are defined anywhere, apply a safe default.
	// This allows GET by default and denies everything else.
	else
	{
		if (method == "GET")
			return;
...
std::pair<std::string, std::string> HttpParser::parseHost(std::string const &str)
{
	...
	if (found != std::string::npos)
	{
		...
		if (port.empty())
			throw std::invalid_argument(E_400); // si hay localhost: dos puntos sin puerto
		if (!isInt(port))
			throw std::invalid_argument(E_400);
		if (port[0] == '0')
			throw std::invalid_argument(E_400);
