...
/*   Updated: 2025/09/14 13:25:32 by fdi-cecc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool HttpParser::notImplementedMethod(std::string const &method) {
		for (int i = 0; i < valid_method_count; i++)
				if (method == valid_method[i]) return true;
		return false;
}

ServerData const &HttpParser::checkIfServerExist(
	std::vector<ServerData> const &servers,
	std::pair<int, std::string> incoming) {
		...
		for (it = servers.begin(); it != ite; ++it) {
				...
				for (it_listen = listens.begin(); it_listen != ite_listen;
					 ++it_listen) {
						if (*it_listen == incoming) return *it;
				}
		}
...
// void	HttpParser::checkIfPathExist( std::pair<std::string, std::string>  const
// & path ) { 	std::string full( path.first + path.second );
...
std::pair<std::string, std::string> HttpParser::parseHost(
...
std::pair<std::string, std::string> HttpParser::parseHeaderSyntaxis(
	std::string h) {
...
std::vector<std::string> HttpParser::pushValues(std::string n, std::string v) {
		...
		for (it = values.begin(); it != ite; ++it) trimSpaceAndTab(*it);
...
void HttpParser::pushMoreValues(
	...
	std::string v) {
		...
		for (it = more_values.begin(); it != ite; ++it) trimSpaceAndTab(*it);
...
std::string HttpParser::parseContentTypeBoundary(
	std::vector<std::string> const &v) {
		...
		for (it = boundary.begin(); it != ite; ++it)
				if (*it > 126 || *it < 33) throw std::invalid_argument(E_400);
