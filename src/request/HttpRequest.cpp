...
HttpRequest::HttpRequest(std::pair<int, std::string> incoming,
						 ServerManager &server)
	: req_line(NULL), uri(NULL), headers(NULL), body(""), body_len(0),
	  boundary(""), boundary_flag(false), code(200), state(SKIP),
	  incoming(incoming), server(server)
{
		std::cout << "INCOMING FIRST: " << incoming.first
				  << " SECOND: " << incoming.second << std::endl;
...
HttpRequest &HttpRequest::operator=(const HttpRequest &rhs)
{
		if (this != &rhs)
		{
				...
				this->req_line =
					rhs.req_line ? new RequestLine(*rhs.req_line) : NULL;
				if (this->uri)
						delete this->uri;
...
void HttpRequest::sendBuffer(char *buffer, ssize_t bytes)
{
		...
		try
		{
				while (found != std::string::npos && this->state != BODY)
				{
						...
						case SKIP:
								if (tmp.empty())
										...
										this->state = REQ_LINE;
						case REQ_LINE:
								this->req_line = new RequestLine(
									HttpParser::parseRequestLine(tmp));
								this->state = HEADERS;
								break;
						case HEADERS:
								if (tmp.empty())
				...
				if (this->state == BODY)
				{
						if (!this->boundary.empty() &&
							this->getRspType() != "cgi-script")
								manyBodiesRoutine(found);
						// For simple bodies OR for ANY request going to a CGI
						// script, treat the entire body as a single raw string.
						else
						{
								// Append the received data to the body and
								// clear the temporary buffer. This is safer
								// than 'this->body = this->fullRequest' if data
								// arrives in chunks.
		...
		if (this->state == DONE)
		{
				if (getRspType() == "cgi-script")
				{
						...
						// std::cout << RED << __func__ << " [status code check]
						// " << this->code << RESET << std::endl; // DBG
		...
		//	this->state = DONE; //solo poner en caso de debug para que no se
		// quede colgado
}
...
void HttpRequest::printBodies() // DBG
{
		...
		for (std::vector<MultiBody>::const_iterator it = _bodies.begin();
			 it != _bodies.end(); ++it)
		{
				...
				std::map<std::string, std::vector<std::string>>::const_iterator
					cd_it = it->bodyHeader.getHeader("content-disposition");
				...
				if (cd_it != it->bodyHeader.getHeaderEnd())
				{
						...
						if (filename_pos != std::string::npos)
						{
								// Extract filename
								size_t start_quote =
									contDisp.find("\"", filename_pos);
								size_t end_quote =
									contDisp.find("\"", start_quote + 1);
...
LocationConf const *
HttpParser::findLocation(std::string path,
						 std::map<std::string, LocationConf> const &loc)
{
		...
		for (it = loc.begin(); it != loc.end(); it++)
		{
				...
				{
						if (tmp == loc.end() ||
							tmp->first.length() < it->first.length())
								tmp = it;
				}
		}
		if (tmp == loc.end())
...
void HttpRequest::finalHeadersParsingRoutine()
{
		...
		//	std::cout << "FULLPATH: " << this->_fullPath.first << " " <<
		// this->_fullPath.second << std::endl; 	std::cout << "PATH: " <<
		// this->uri->getPath() << std::endl;
		setLocation(serv.getLocations(), this->_fullPath.second);

		// HttpParser::notAllowedMethod(serv.getItLocations(this->location),
		// serv.getAllowedMethods(),
		// this->req_line->getMethod());

		blockLoc = findLocation(this->_fullPath.second, serv.getLocations());
		// std::cout << "BLOCK LOCK FINAL HEADERS: " <<
		// blockLoc.getErrorPage().begin()->second << std::endl;
		HttpParser::checkIfPathExist(this->_fullPath, blockLoc,
									 this->getHttpMethod());
			...
			this->headers->getHeaderEnd())
		{
				this->body_len = HttpParser::parseContentLengthHeader(
					this->headers->getHeaderOnlyOneValue("content-length", 0),
					this->max_body_size);
				this->state = BODY;
		}
		if (this->headers->getHeader("content-type") !=
				this->headers->getHeaderEnd() ||
			this->headers->getHeader("content-length") !=
				this->headers->getHeaderEnd())
				return;
		this->state = DONE;
}
...
void HttpRequest::manyBodiesRoutine(std::size_t found)
{
		...
		static bool makeNew = true; // FABIO created boolean so that it only
									// creates the bodies when needed
		...
		while (found != std::string::npos && this->body_state != BODY2)
		{
				...
				case BOUNDARY:
							...
							tmp != "--" + this->boundary)
						{
								...
								throw std::invalid_argument(
									E_400); // si el primer boundary no
											// corresponde: error
						}
						// FABIO create new struct body
						this->boundary_flag = false;
						// k++;
						// std::cout << std::endl << GREEN << " NEWBODY " << k
						// << RESET << std::endl;
				...
				case HEADERS2:
						...
						// std::cout << "HEADER " << k << ": " << tmp.substr(0,
						// 70)
						// 		  << std::endl; // FABIO
						// structbody->header->setHeader( tmp )
		...
		{
				this->body = this->body.erase(
					f, this->body.length()); // FABIO poner body en la struct
											 // del ultimo body
				newBody.bodyContent = this->body;
				// std::cout << "BODY   " << k << ": " << this->body.substr(0,
				// 70) << std::endl;
		...
		{
				this->body = this->body.erase(
					f2, this->body.length()); // FABIO poner body  en la struct
				newBody.bodyContent = this->body;
				// std::cout << "BODY   " << k << ": " << this->body.substr(0,
				// 70) << std::endl;
				this->_bodies.push_back(newBody);
...
void HttpRequest::setLocation(
	std::map<std::string, LocationConf> const &locations,
	std::string const &path)
{
		std::cout << PINK << "request path: " << path << std::endl
				  << RESET; // TO BORROW

		std::map<std::string, LocationConf>::const_iterator best_match =
			locations.end();

		for (std::map<std::string, LocationConf>::const_iterator it =
				 locations.begin();
			 it != locations.end(); ++it)
				if (path.find(it->first) == 0)
		...
		std::cout << PINK << "MATCHED LOCATION: " << this->location
				  << std::endl;
		...
		if (_rspType == "cgi-script")
		{
				...
				if (dotPos != std::string::npos)
				{
						...
						std::map<std::string, std::string>::const_iterator
							cgi_it = cgiPassMap.find(extension);

						if (cgi_it != cgiPassMap.end())
						{
								this->_cgiInterpreterPath = cgi_it->second;
								std::cout
									<< GREEN << "Found CGI interpreter for "
									<< extension << ": " << _cgiInterpreterPath
									<< RESET << std::endl;
						} else
						{
...
void HttpRequest::setRspType()
{
		...
		if (extension == ".html" || extension == ".htm")
				...
				_rspType = "text/css";
				 ...
				 extension == ".php") // TODO check if valid
				_rspType = "cgi-script";
		else
				_rspType = "application/octet-stream";
}

void HttpRequest::checkHost(
	...
	ServerData &serv)
{
		...
		this->host = HttpParser::parseHost(
			this->headers->getHeaderOnlyOneValue("host", 0));

		if (!HttpParser::checkIfHostNameExistInServer(this->host.first,
													  serv.getServerName()))
				throw std::invalid_argument(E_400);
}
...
{
		_fullPath = std::make_pair<std::string, std::string>(serv.getRoot(),
															 uri->getPath());
}
...
void HttpRequest::fileUpload()
{
			 ...
			 it != _bodies.end(); ++it)
		{
				...
				if (filename_pos != std::string::npos)
				{
						...
						std::string nameOriginal = contDisp.substr(
						...
						std::string uploadPath =
							_fullPath.first + _fullPath.second + nameClean;

						// Save the file
						std::ofstream fileUp(uploadPath.c_str(),
											 std::ios::binary);
						if (fileUp.is_open())
						{
								fileUp << it->bodyContent;
								fileUp.close();
								std::cout << GREEN << "SUCCESS: Saved file to "
										  << uploadPath << RESET << std::endl;
						} else
						{
								std::cerr << "ERROR: Could not open file for "
											 "writing: "
										  << uploadPath << std::endl;
		...
		/* 	TODO Generate a Response: After processing, set a success status
		code. The Response class will then use this to build the final reply.
...
LocationConf
HttpRequest::findLocation(std::string path,
						  std::map<std::string, LocationConf> const &loc)
{
		std::map<std::string, LocationConf>::const_iterator it;
		std::map<std::string, LocationConf>::const_iterator tmp = loc.end();
